...
func RaylibApp() {
	...
	heightMap := pkg.NewPerlinMap(256, 50)

	image := rl.NewImageFromImage(heightMap)  // Load heightmap image (RAM)
	...
	fmt.Println(mesh.VertexCount) // Load model from generated mesh
	...
	rl.SetCameraMode(camera, rl.CameraFree) // Set an orbital camera mode
